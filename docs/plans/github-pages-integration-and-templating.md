...
- [x] Add support for section-specific parameters (filter, aggregate-by, separate-by, depth, title, show-epochs, detect-changes)
...
- Per-section control via `show-epochs` and `detect-changes` parameters (OR'd with global CLI flags)
