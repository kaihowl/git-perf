...
* `--max-cov <MAX_COV>` — Flag measurements with high Coefficient of Variation (CoV = σ/μ × 100%). When tail or head CoV exceeds this percentage threshold, a warning is shown in the audit output indicating low measurement stability. If not specified, uses the value from .gitperfconfig file, or no flagging
