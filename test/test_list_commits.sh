...
if [ -n "$output" ]; then
  test_section "Expected empty output for repository without measurements"
  exit 1
fi
popd
...
if [ "$line_count" -ne 2 ]; then
  test_section "Expected 2 lines of output, got $line_count"
  exit 1
fi
popd
...
output=$(git perf list-commits)
if [ -n "$output" ]; then
  test_section "Expected empty output after removing all measurements"
  test_section "Got: $output"
  exit 1
fi
popd
popd
