...
# Higher values increase the number of iterations for more thorough testing
ITERATION_SCALE_FACTOR=${ITERATION_SCALE_FACTOR:-1}

# Base iteration counts (multiplied by ITERATION_SCALE_FACTOR)
BASE_PUSH_ITERATIONS=150
BASE_REMOVE_ITERATIONS=150
BASE_PRUNE_ITERATIONS=100
BASE_ADD_ITERATIONS=199

# Calculate actual iteration counts based on scale factor
NUM_PUSH_ITERATIONS=$((BASE_PUSH_ITERATIONS * ITERATION_SCALE_FACTOR))
NUM_REMOVE_ITERATIONS=$((BASE_REMOVE_ITERATIONS * ITERATION_SCALE_FACTOR))
NUM_PRUNE_ITERATIONS=$((BASE_PRUNE_ITERATIONS * ITERATION_SCALE_FACTOR))
NUM_ADD_ITERATIONS=$((BASE_ADD_ITERATIONS * ITERATION_SCALE_FACTOR))

# Concurrency settings (NOT affected by scale factor)
CONCURRENT_ADDERS=4
