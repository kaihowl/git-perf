...
if echo "$output" | grep -q "bench_"; then
  test_section "FAIL: bench_ measurements should not be audited"
  exit 1
fi
if echo "$output" | grep -q "test_"; then
  test_section "FAIL: test_ measurements should not be audited"
  exit 1
fi
test_section "PASS: Filter-only successfully audited perf_* measurements"
...
if echo "$output" | grep -q "perf_"; then
  test_section "FAIL: perf_ measurements should not be audited"
  exit 1
fi
test_section "PASS: Filter-only successfully audited bench_* measurements"

test_section "Test 3: Audit with unanchored pattern (matches substring)"
...
test_section "PASS: Unanchored pattern works correctly"

test_section "Test 4: Error when neither -m nor --filter provided"
if output=$(git perf audit -d 10 2>&1); then
  test_section "FAIL: Should error when neither -m nor --filter provided"
  exit 1
fi
assert_contains "$output" "required arguments" "Should indicate missing required arguments"
test_section "PASS: Correctly errors when both -m and --filter are missing"

test_section "All filter-only tests passed successfully"
test_stats
exit 0
