...
test_section "Created repository with 6 commits"

# Add performance measurements across different commits for TWO different metrics
test_section "Adding performance measurements for build_time and memory_usage..."
...
test_section "Added measurements for build_time and memory_usage across 6 commits"

# Test 1: Different min_measurements per measurement
test_section "Test 1: Different min_measurements per measurement"
...
test_section "Test 2: Different aggregate_by per measurement"
cat > .gitperfconfig << 'EOF'
[measurement]
aggregate_by = "median"
...
assert_failure_with_output AUDIT_BUILD_AGG git perf audit -m build_time -n 10
assert_failure_with_output AUDIT_MEMORY_AGG git perf audit -m memory_usage -n 10
...
test_section "Test 3: Different sigma per measurement"
cat > .gitperfconfig << 'EOF'
[measurement]
sigma = 4.0
...
assert_failure_with_output AUDIT_BUILD_SIGMA git perf audit -m build_time -n 10
assert_failure_with_output AUDIT_MEMORY_SIGMA git perf audit -m memory_usage -n 10
...
test_section "Test 4: Multiple measurements with different dispersion methods in single audit"
cat > .gitperfconfig << 'EOF'
[measurement]
...
assert_failure_with_output AUDIT_MULTI git perf audit -m build_time -m memory_usage -n 10
...
test_section "Test 5: CLI option overrides per-measurement config"
cat > .gitperfconfig << 'EOF'
[measurement."build_time"]
min_measurements = 10
aggregate_by = "max"
...
assert_failure_with_output AUDIT_CLI_OVERRIDE git perf audit -m build_time -n 10 --min-measurements 2 -a min -d 3.0 --dispersion-method stddev
...
assert_failure_with_output AUDIT_CLI_MIN_ALL git perf audit -m build_time -m memory_usage -n 3 --min-measurements 2
...
test_section "Test 6: Three measurements with different configs for all parameters"
...
assert_failure_with_output AUDIT_THREE git perf audit -m build_time -m memory_usage -m test_metric -n 10
...
test_section "Test 7: Falls back to global config when measurement-specific not defined"
cat > .gitperfconfig << 'EOF'
[measurement]
min_measurements = 4
aggregate_by = "mean"
...
assert_failure_with_output AUDIT_FALLBACK git perf audit -m build_time -m memory_usage -n 10
...
test_section "Test 8: Warning when max_count < min_measurements from config"
cat > .gitperfconfig << 'EOF'
[measurement."build_time"]
min_measurements = 10
...
assert_failure_with_output AUDIT_WARNING git perf audit -m build_time -m memory_usage -n 3
...
test_section "Test 9: No warning when max_count >= min_measurements"
cat > .gitperfconfig << 'EOF'
[measurement."build_time"]
min_measurements = 3
EOF
...
assert_failure_with_output AUDIT_NO_WARNING git perf audit -m build_time -n 5

# Verify no warning appears
if echo "$AUDIT_NO_WARNING" | grep -q "Warning.*max_count"; then
    echo "❌ Unexpected warning when max_count >= min_measurements"
    test_section "Output was: $AUDIT_NO_WARNING"
    exit 1
fi
...
test_section "Test 10: CLI validation prevents invalid max_count/min_measurements combination"
...
if echo "$AUDIT_CLI_INVALID" | grep -q "limits available historical data"; then
    ...
    test_section "Output was: $AUDIT_CLI_INVALID"
    exit 1
fi
echo "✅ CLI validation prevents invalid combinations before runtime"

test_section "All per-measurement configuration tests passed!"
