...
# Improved script to validate manpage.md using consistent pandoc options
# This addresses the root causes instead of complex post-processing

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
WORKSPACE_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"

echo "🔍 Validating manpage.md with consistent pandoc formatting..."
...
# Generate the expected manpage using consistent pandoc options
echo "📝 Generating expected manpage with consistent pandoc formatting..."
TEMP_MANPAGE="/tmp/expected_manpage.md"

# Use the same consistent pandoc options as the generation script
for file in "${EXPECTED_FILES[@]}"; do
...
if diff -u docs/manpage.md "$TEMP_MANPAGE" > /tmp/manpage.diff; then
    ...
    echo "   ./scripts/generate-manpage-improved.sh"
    echo ""
