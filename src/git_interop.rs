...
pub fn remove_measurements_from_commits(older_than: DateTime<Utc>) -> Result<()> {
    let op = || -> Result<(), backoff::Error<GitError>> {
        raw_remove_measurements_from_commits(older_than).map_err(|e| match e {
            // TODO(kaihowl) think these through
            GitError::RefConcurrentModification { .. }
            | GitError::RefFailedToLock { .. }
            | GitError::RefFailedToPush { .. } => backoff::Error::transient(e),
            GitError::ExecError { .. }
            | GitError::IoError { .. }
            | GitError::MissingHead { .. } => backoff::Error::permanent(e),
        })
    };
