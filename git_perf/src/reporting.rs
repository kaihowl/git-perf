...
mod tests {
    ...
    #[test]
    fn test_broken_pipe_error_handling() {
        use std::io::{Error, ErrorKind};

        // Simulate the BrokenPipe error handling logic
        let broken_pipe_error = Error::new(ErrorKind::BrokenPipe, "broken pipe");
        let result = if broken_pipe_error.kind() == ErrorKind::BrokenPipe {
            Ok(())
        } else {
            Err(broken_pipe_error)
        };

        // BrokenPipe should result in Ok(())
        assert!(result.is_ok());

        // Other errors should propagate
        let other_error = Error::new(ErrorKind::PermissionDenied, "permission denied");
        let result = if other_error.kind() == ErrorKind::BrokenPipe {
            Ok(())
        } else {
            Err(other_error)
        };
    ...
    #[test]
    fn test_stdout_write_error_kinds() {
        use std::io::{Error, ErrorKind};

        // Test that only BrokenPipe is treated as success
        let test_cases = vec![
            (ErrorKind::BrokenPipe, true),        // Should be Ok
            (ErrorKind::PermissionDenied, false), // Should be Err
            (ErrorKind::NotFound, false),         // Should be Err
            (ErrorKind::ConnectionReset, false),  // Should be Err
            (ErrorKind::WriteZero, false),        // Should be Err
        ];

        for (kind, should_succeed) in test_cases {
            let error = Error::new(kind, "test error");
            let result = if error.kind() == ErrorKind::BrokenPipe {
                Ok(())
            } else {
                Err(error)
            };
            ...
            if should_succeed {
                assert!(result.is_ok(), "ErrorKind::{:?} should be Ok", kind);
            } else {
                assert!(result.is_err(), "ErrorKind::{:?} should be Err", kind);
            }
        }
    }
}
