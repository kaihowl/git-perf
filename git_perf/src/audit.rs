...
mod test {
    ...
    #[test]
    fn test_audit_result_insufficient_measurements_includes_header() {
        // Test that audit result with insufficient measurements includes measurement name header

        // Create an AuditResult for insufficient measurements case
        let measurement_name = "test-measurement";
        let number_measurements = 2;
        let min_count = 10;
        let plural_s = "s";

        let result = AuditResult {
            message: format!("⏭️ '{measurement_name}'\nOnly {number_measurements} measurement{plural_s} found. Less than requested min_measurements of {min_count}. Skipping test."),
            passed: true,
        };

        // Verify the message includes the header with measurement name
        assert!(result.message.starts_with("⏭️ 'test-measurement'"));
        assert!(result.message.contains("Only 2 measurements found"));
        assert!(result
            .message
            .contains("Less than requested min_measurements of 10"));
        assert!(result.message.contains("Skipping test"));
        assert!(result.passed);

        // Test different measurement counts
        let result_singular = AuditResult {
            message: format!("⏭️ 'single-measurement'\nOnly 1 measurement found. Less than requested min_measurements of 10. Skipping test."),
            passed: true,
        };

        assert!(result_singular
            .message
            .starts_with("⏭️ 'single-measurement'"));
        assert!(result_singular.message.contains("Only 1 measurement found"));
    }

    #[test]
    fn test_audit_result_header_format_consistency() {
        // Test that the header format is consistent with other audit result types

        let skip_message = "⏭️ 'test-metric'\nOnly 2 measurements found. Less than requested min_measurements of 10. Skipping test.";
        let pass_message = "✅ 'test-metric'\nz-score (stddev): → 0.00\nHead: μ: 100.000 σ: 0.000 MAD: 0.000 n: 1\nTail: μ: 100.000 σ: 0.000 MAD: 0.000 n: 10\n [-0.0% – +0.0%] ▅▅▅▅▅▅▅▅▅▅▅";
        let fail_message = "❌ 'test-metric'\nHEAD differs significantly from tail measurements.\nz-score (stddev): ↑ 2.50\nHead: μ: 150.000 σ: 0.000 MAD: 0.000 n: 1\nTail: μ: 100.000 σ: 10.000 MAD: 5.000 n: 10\n [-10.0% – +50.0%] ▅▅▅▅▅▅▅▅▅█";

        // All messages should start with emoji and measurement name in quotes
        assert!(skip_message.starts_with("⏭️ 'test-metric'"));
        assert!(pass_message.starts_with("✅ 'test-metric'"));
        assert!(fail_message.starts_with("❌ 'test-metric'"));

        // All should have measurement name in single quotes
        assert!(skip_message.contains("'test-metric'"));
        assert!(pass_message.contains("'test-metric'"));
        assert!(fail_message.contains("'test-metric'"));
    }
}
